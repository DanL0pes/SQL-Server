...
EXEC spInsereProduto 'Risoles de jaca', 29, 400
...
EXEC spInsereCliente 'Flávia Regina Brito', '22/04/1992', 'Rua Mariano Moro', 300, '082001', 'Itaqueraaaa', '135498','F'
...
AS
	IF NOT EXISTS(SELECT codCliente FROM  tbCliente WHERE codCliente=@codCliente)
		BEGIN
			PRINT('Não foi possível efetivar a encomenda pois o cliente do código '+CONVERT(VARCHAR(5), @codCliente)+' não está cadastado.')
		END
	ELSE
		BEGIN
...
EXEC spInsereEncomenda '05/10/2015', 4		, 150, '12/10/2015', '9687399'
...
EXEC spInsereItensEncomenda 100, 1, 2.5, 105
...
EXEC spInsereItensEncomenda 500, 6, 3.4, 150
...
AS
	...
	IF NOT EXISTS (SELECT cpfCliente FROM tbCliente WHERE cpfCliente = @cpfCliente)
		BEGIN
			PRINT('Impossível remover esse cliente! Portador do cpf '+@cpfCliente+' não cadastrado.')
		END
	ELSE
